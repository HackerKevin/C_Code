...
// 操作结果：降序插入元素e
{
	...
	DblNode<ElemType>* temp = NULL;//临时变量保存尾节点
	node = new DblNode<ElemType>(e);	  // 生成新结点node
	assert(node);						  // 申请结点失败，终止程序运行 

	if (node->data > head->data)
	{
		temp = head->prior->prior;
		cur_node->prior = node;		//插入位置的前一节点的指针指向新节点
		node->prior = pre_node;		//新节点的prior指向前一节点

		node->next = cur_node;		//新节点的指针指向后一节点
		pre_node->prior = node;		//后一节点的prior指向新节点

		head = node;

		head->prior->next = head;	//形成闭环
		head->prior->prior = temp;
	}
	else
	{
		while (cur_node->data != -1 && e < cur_node->data) //指针没有指到链表结尾且插入值小于当前节点值或当前节点没有值
		{
		...
		node->next = cur_node;		//新节点的指针指向后一节点
		pre_node->next = node;		//后一节点的prior指向新节点
